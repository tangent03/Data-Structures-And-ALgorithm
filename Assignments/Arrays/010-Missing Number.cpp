...
// are in the range [0,2]. 2 is the missing number in the range 
// since it does not appear in nums.
// Example 3:
// 
...
// are in the range [0,9]. 8 is the missing number in the range 
// since it does not appear in nums.
//  
// 
// Constraints:
...
int xorMethod(vector<int>&nums){
	...
	//xor for all range of items [0,N];
	for(int i=0;i<=nums.size();i++){
		ans=ans^i;
	}
	return ans;
...
int main() {
	...
	cout<<"Missing Number Is "<<xorMethod(nums)<<endl;
